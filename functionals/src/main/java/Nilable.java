...
/** Container of [0..1] element.<br/>
  ...
  * To avoid ambiguity, use {@link #isHasValue(Predicate)} or {@code instanceof} pattern match then contained object's {@link Object#equals(Object)}.<br/>
  *
  * <h2>Quick tour</h2>
  * Similar to {@link Optional}, provides a way to represent with the idea of "possible absence of value" but also provide APIs to manipulate them safely &amp; arbitrary nestable.<br/>
  * Most APIs has eager &amp; lazy-variant overloads.<br/>
  * API naming scheme follows hierarchical namespace-style &amp; any sub-namespace <b>must preserve</b> super-namespace primary behavior.<br/>
  * For example, {@link #to(Function)} "super-namespace" has family of methods with {@code to-} prefix that also preserve the idea of "transform this {@link Nilable} to other type".<br/><br/>
  * 
  * API families:<ul>
  *     <li>Factory functions: {@code of-} prefix, {@link #empty()}, {@link #from(Optional)}</li>
  *     <li>Boolean operators on {@link Nilable} type: {@link #and(Nilable)}, {@link #or(Nilable)}, {@link #xor(Nilable)}</li>
  *     <li>Boolean operators on contained type {@code T}: {@code is-} prefix, {@link #isHasValue()}</li>
  *     <li>Algebraic operators: {@code map-} prefix &amp; {@link #flatMap(Function)}</li>
  *     <li>Type transformation: {@code to-} prefix</li>
  *     <li>Side-effect operations: {@code peek-} prefix</li>
  *     <li>Unwrapping: {@code unwrap-} prefix, {@link #orElse(Object)}, {@link #zDangerouslyUnwrap(String)}</li>
  * </ul>
  *
  * <h3>Note</h3>
  * This type deliberately designed to not provide any API that produces raw {@code null}.<br/>
  * In case it's required for interfacing with external library, use {@code instanceof} or {@code switch} pattern match externally and manually return {@code null}.
  *
  * @param <T> Any type
  *
...
public sealed interface Nilable<T> extends Transmutable<Nilable<T>> { // Post-Valhalla: public sealed abstract value class Nilable<T>
    ...
    /** Transpose-{@link Faulty}: Bijective map {@code Faulty<Nilable<T>,E> -> Nilable<Faulty<T,E>>}.
      ...
      *     <tbody>
      *         <tr><td>{@code Faulty.Ok(Nilable.Has(value))}</td><td>{@code Nilable.Has(Faulty.Ok(value))}</td></tr>
      *         <tr><td>{@code Faulty.Ok(Nilable.Empty)}</td>     <td>{@code Nilable.Empty}</td></tr>
      *         <tr><td>{@code Faulty.Error(error)}</td>          <td>{@code Nilable.Has(Faulty.Error(error))}</td></tr>
      *     </tbody>
      * </table>
      * @param <T> Any type
